...
class DataProcessor:
	...
	def __init__(
		...
		self._core_model, self.__delta_model = core_model, delta_model
		...
		self._seq_len = self._core_model.input_shape[1]
		if depth_input:
			self._seq_len -= 1
	...
	def _forecast(self, sequence, depth, initial_depth=0) -> np.ndarray:
		...
		for i in range(initial_depth, depth):
			...
			directions = self._core_model.predict(core_input)
	...
	def _apply_depth(self, sequence: np.ndarray, depth) -> np.ndarray:
		start_depth = 0
		...
		if cached is not None:
			...
			start_depth = depth - 1
		forecast = self._forecast(input_sequence, depth, initial_depth=start_depth)
		self.__cache.store(sequence, depth, forecast)
		return forecast

	def _process_batch(self, batch: np.ndarray, depth) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
		...
		core_x[:, :self._seq_len] = delta_x[:, :self._seq_len] = input_sequence

		if self._depth_input:
			core_x[:, -1] = delta_x[:, -1] = depth

		core_y[:] = delta_y[:] = batch[:, self._seq_len + depth] - input_sequence[:, -1]
		core_y[core_y <= 0], core_y[core_y > 0] = 0, 1
		delta_y[:] = np.abs(delta_y)

		delta_x[:, self._seq_len] = core_y
		if not self.__true_directions:
			delta_x[:, self._seq_len] = self._core_model.predict(core_x).flatten()
			if self.__direction_rounding:
				delta_x[:, self._seq_len] = np.round(delta_x[:, -2])
	...
	def get_data(self, idx, depth) -> Tuple[WrapperGenerator, WrapperGenerator]:
		...
		for i in range(rounds):
			core_batch, delta_batch = self._process_batch(
