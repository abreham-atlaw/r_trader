...
class DataPool:
	...
	def __init__(self, size: int):
		...
		self.__pool_idxs = (torch.zeros(self.size, dtype=torch.int32) - 1).share_memory_()

	def __init_pool(self, X: torch.Tensor, y: torch.Tensor):
		self.__pool_X = torch.zeros((self.size, *X.shape)).share_memory_()
		self.__pool_y = torch.zeros((self.size, *y.shape)).share_memory_()
